/*
    Author: Joris-Jan van 't Land (original script), Rimantas Galvonas (modification to use ATL instead of ASL)
...

{
    ...
    {
        private ["_type", "_relPos", "_azimuth", "_fuel", "_damage", "_orientation", "_varName", "_init", "_simulation", "_ATL", "_newObj"];
        _type = _x select 0;
        _relPos = _x select 1;
        ...
        if (!_ATL) then {_newObj setPos _newPos;} else {_newObj setPosATL _newPos; _newObj setVariable ["BIS_DynO_ASL", true];};
