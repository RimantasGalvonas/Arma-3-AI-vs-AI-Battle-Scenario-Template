/*
    Author:
        Joris-Jan van 't Land, optimised by Killzone_Kid, debug mode added by Rimantas Galvonas
...
//#define arr1 [_checkPos,_minDistance,_maxDistance,_objectProximity,_waterMode,_maxGradient,_shoreMode,_posBlacklist,_defaultPos,_debug]
//#define arr2 [[],0,0,0,0,0,0,[],[],false]
...
if (_debug && {isNil "Rimsiakas_findSafePosDebugMarkers"}) then {
    Rimsiakas_findSafePosDebugMarkers = [];
};
...
if (_debug) then {
    _markerName = "Rimsiakas_findSafePosDebugCenterMarker" + str (count Rimsiakas_findSafePosDebugMarkers);
    createMarkerLocal [_markerName, _checkPos];
    _markerName setMarkerShapeLocal "ELLIPSE";
    _markerName setMarkerColorLocal "ColorBlue";
    _markerName setMarkerAlphaLocal 0.5;
    _markerName setMarkerBrushLocal "DiagGrid";
    _markerName setMarkerSizeLocal [_maxDistance, _maxDistance];
    Rimsiakas_findSafePosDebugMarkers append [_markerName];

    _markerName = _markerName + "size";
    createMarkerLocal [_markerName, _checkPos];
    _markerName setMarkerTypeLocal "mil_dot";
    _markerName setMarkerColorLocal "ColorWhite";
    _markerName setMarkerTextLocal (str _maxDistance);
    Rimsiakas_findSafePosDebugMarkers append [_markerName];

    if (_minDistance > 0) then {
        _markerName = "Rimsiakas_findSafePosDebugCenterMarkerMegative" + str (count Rimsiakas_findSafePosDebugMarkers);
        createMarkerLocal [_markerName, _checkPos];
        _markerName setMarkerShapeLocal "ELLIPSE";
        _markerName setMarkerColorLocal "ColorRed";
        _markerName setMarkerAlphaLocal 0.5;
        _markerName setMarkerBrushLocal "DiagGrid";
        _markerName setMarkerSizeLocal [_minDistance, _minDistance];
        Rimsiakas_findSafePosDebugMarkers append [_markerName];

        _markerName = _markerName + "negativeSize";
        createMarkerLocal [_markerName, [_checkPos select 0, (_checkPos select 1) - 100]];
        _markerName setMarkerTypeLocal "mil_dot";
        _markerName setMarkerColorLocal "ColorWhite";
        _markerName setMarkerTextLocal str (0 - _minDistance);
        Rimsiakas_findSafePosDebugMarkers append [_markerName];
    };
};
...
for "_i" from 1 to MAX_TRIES do
{
    ...
    {
        if (_debug) then {
            _markerName = "Rimsiakas_findSafePosDebugMarker" + str (count Rimsiakas_findSafePosDebugMarkers);
            createMarkerLocal [_markerName, _this];
            _markerName setMarkerTypeLocal "mil_dot";
            _markerName setMarkerColorLocal "ColorRed";
            Rimsiakas_findSafePosDebugMarkers append [_markerName];
        };
        ...
        if (_debug) then {
            _markerName = "Rimsiakas_findSafePosDebugMarker" + str (count Rimsiakas_findSafePosDebugMarkers - 1);
            createMarkerLocal [_markerName, _this];
