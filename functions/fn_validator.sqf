...
if (isNil "patrolCenter" || {isNil {patrolCenter getVariable "patrolRadius"} || isNil {patrolCenter getVariable "intelGridSize"}}) then {
    ...
    if ((count _placers) == 0) then {
        ...
        if (_hasProperlyConfiguredPlacers == false) then {
            _placerHints2 append [parseText "The simplest way to make a placer spawn units is to place a unit or a group in the editor and sync it to the placer (sync only one unit from the group, not all of them). This will randomize that unit's position according to the placer and make them roam the mission area."];
